...
async function verifyEvaluator() {
    ...
    draws.map(draw => async function() {
        draw.review.map(review => async function (){
...
async function changeEvaluator(evaluator, draw){
    ...
    await Draw.findOneAndUpdate(draw._id, {$pull: {review: {evaluator: evaluator._id}}});
    //remove o desenho da lista de desenhos do avaliador atual
    await Evaluator.findOneAndUpdate(evaluator._id, {$pull: {draws: draw._id}});
    //adiciona o novo avaliador no desenho
    await Draw.findOneAndUpdate(draw._id, {$push: {review: {evaluator: newEvaluator._id}}});
    //adiciona o desenho na lista de desenhos do novo avaliador
    await Evaluator.findOneAndUpdate(newEvaluator._id, {$push: {draws: draw._id}});
}
